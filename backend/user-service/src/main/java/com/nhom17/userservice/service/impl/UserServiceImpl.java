...
import com.nhom17.userservice.constant.KafkaConstant;
import com.nhom17.userservice.event.EventProducer;
import com.nhom17.userservice.exception.wrapper.*;
import com.nhom17.userservice.model.dto.request.*;
import com.nhom17.userservice.model.dto.response.InformationMessage;
import com.nhom17.userservice.model.dto.response.JwtResponseMessage;
import com.nhom17.userservice.model.dto.response.RegisterMessage;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Mono<RegisterMessage> register(SignUp signUp) {
        return Mono.defer(() -> {
            if (existsByUsername(signUp.getUsername())) {
                return Mono.error(new EmailOrUsernameNotFoundException("The username " + signUp.getUsername() + " is already taken, please try again."));
            }
            if (existsByEmail(signUp.getEmail())) {
                return Mono.error(new EmailOrUsernameNotFoundException("The email " + signUp.getEmail() + " is already registered, please try again."));
            }
            if (existsByPhoneNumber(signUp.getPhone())) {
                return Mono.error(new PhoneNumberNotFoundException("The phone number " + signUp.getPhone() + " is already registered, please try again."));
            }

            // Map SignUp to User entity
            ...
            // Save the user and return a Mono
            return Mono.fromCallable(() -> userRepository.save(user)) // Ensures the save is wrapped in Mono
                .flatMap(savedUser -> {
                    // Create UserPrinciple and Authentication
                    ...
                    // Prepare RegisterMessage with access token
                    RegisterMessage registerMessage = RegisterMessage.builder()
                        .accessToken(accessToken) // Set access token
                        .message("Create user: " + signUp.getUsername() + " successfully.") // Set success message
                        .build();

                    // Return the response in a Mono
    ...
    public Mono<JwtResponseMessage> login(Login signInForm) {
        ...
        // Kiểm tra mật khẩu
        if (!passwordEncoder.matches(signInForm.getPassword(), userDetails.getPassword())) {
            throw new PasswordNotFoundException("Incorrect password. Please try again.");
        }
        ...
        return Mono.just(jwtResponseMessage); // Trả về JwtResponseMessage trong Mono
    }
    ...
    public String textSendEmailChangePasswordSuccessfully(String username) {
        return "Hey " + username + "!\n\n" +
                ...
                "Best regards:\n\n" +
                "Contact: nhom17qx1@gmail.com\n" +
                "Fanpage: https://nhom17qx1.github.io/";
    }
