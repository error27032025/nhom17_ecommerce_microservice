package com.nhom17.userservice.security.jwt;
...
public class JwtProvider {
    ...
    public String createTokenFromUser(User user) {
        List<String> authorities = user.getRoles().stream()
            .map(role -> role.getName().name())  // Lấy tên vai trò dưới dạng String
            .collect(Collectors.toList());
        ...
        return Jwts.builder()
            ...
            .claim("authorities", authorities)
            .setIssuedAt(new Date())
            .setExpiration(new Date(new Date().getTime() + jwtExpiration * 1000L))  // jwtExpiration là thời gian hết hạn token
            .signWith(SignatureAlgorithm.HS512, jwtSecret)
            .compact();
    }
}
